...
def detail(request, isbn):
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            print("the click type is",type(click))
            print('click value is',click)
            readIt=0
            review = "messi"
            rating = 0
            try:
                readIt = ast.literal_eval(request.body.decode("utf-8"))
                print("the read it is",readIt)
            except:
                print("except")           
            if(type(readIt)==dict):
            ...
            if(review == None or review == '' or review=="messi"):
                print("---------empty--------review")
            else:
                # print("--------------------------------review given----------------------------")
                user_id= request.user.id + 278858
